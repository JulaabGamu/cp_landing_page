...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false }: BandProps) {
    ...
    const baseTexture = useTexture('/lanyard/lanyard.png');
    ...
    const [curve] = useState(() => {
        const newCurve = new THREE.CatmullRomCurve3([new THREE.Vector3(), new THREE.Vector3(), new THREE.Vector3(), new THREE.Vector3()]);
    ...
    useFrame((state, delta) => {
        ...
        flipRotation.current += (targetRotation - flipRotation.current) * 0.1;
        if (cardGroupRef.current) {
            cardGroupRef.current.rotation.y = flipRotation.current;
        }
    });

    // Click detection helpers
    const handlePointerDown = (e: React.PointerEvent<THREE.Group> & { point: THREE.Vector3; pointerId: number }) => {
        e.target.setPointerCapture(e.pointerId);
        pointerDownPos.current = { x: e.clientX, y: e.clientY };
        drag(new THREE.Vector3().copy(e.point).sub(vec.copy(card.current.translation())));
    };

    const handlePointerUp = (e: React.PointerEvent<THREE.Group>) => {
